...
import type { ShelfStatus, UserBookShelf, Wishlist } from "@/generated/prisma/client";
...
export default function ShelfManager({
  ...
  const updateShelfItem = async (
    ...
    try {
      const data = await fetcher<{ item: UserBookShelf }>(
        `/api/shelf/${id}`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
    ...
    <div className="space-y-10">
      ...
      <section>
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          ...
          <Button size="sm" onClick={() => setShowShelfForm((v) => !v)}>
        ...
        {visibleShelf.length === 0 ? (
          <EmptyState
            ...
            action={
              <Button size="sm" onClick={() => setShowShelfForm(true)}>
                  ...
                  {item.status === "COMPLETED" && (
                    <div className="flex items-center gap-1 text-xs text-stone-400">
                      ...
                      <StarRating
                        value={item.rating ?? 0}
                        onChange={(rating) =>
                          updateShelfItem(item.id, { rating })
                        }
                        size={14}
                      />
                    </div>
